...
SHYFT_DIR = APP_DATA_DIR / 'shyft'

# Ensure the 'shyft' directory exists
ensure_directory(SHYFT_DIR)
...
class ShyftGUI:
    def __init__(self, master):
        ...
        self.data_file_path = SHYFT_DIR / "data.json"
        self.data = {}
        self.load_data()
        self.create_widgets()
        self.refresh_view()
    ...
    def update_styles(self):
        ...
        background = "black" if self.is_dark_mode else self.bg_color
        button_foreground = "white" if self.is_dark_mode else self.btn_text_color
        button_background = "gray" if self.is_dark_mode else "light gray"
        entry_foreground = "white" if self.is_dark_mode else "black"
        entry_background = "black" if self.is_dark_mode else "white"
        ...
        self.style.configure(
            "TEntry", foreground=entry_foreground, background=entry_background, font=("Helvetica", 12)
        )
        self.style.configure(
    ...
    def create_widgets(self):
        ...
        ttk.Button(button_frame, text="Toggle Dark Mode", command=self.toggle_dark_mode, style="TButton").pack(side="left", expand=True)
    ...
    def open_settings(self):
        ...
        settings_window.geometry("140x210")  # Set an appropriate size for the settings window
        ...
        settings_frame.pack(padx=1, pady=1, anchor="w")

        ttk.Label(settings_frame, text="Timestring Color", style="TLabel").pack(anchor="w")
        time_color_button = ttk.Button(settings_frame, textvariable=time_color_var, command=lambda: choose_color(time_color_var, time_color_button), style="TButton")
        time_color_button.pack(fill="x", pady=1)

        ttk.Label(settings_frame, text="Timer Background Color", style="TLabel").pack(anchor="w")
        bg_color_button = ttk.Button(settings_frame, textvariable=bg_color_var, command=lambda: choose_color(bg_color_var, bg_color_button), style="TButton")
        bg_color_button.pack(fill="x", pady=1)

        ttk.Label(settings_frame, text="Timer Button Text Color", style="TLabel").pack(anchor="w")
        btn_text_color_button = ttk.Button(settings_frame, textvariable=btn_text_color_var, command=lambda: choose_color(btn_text_color_var, btn_text_color_button), style="TButton")
        ...
        ttk.Button(settings_frame, text="Save", command=save_settings, style="TButton").pack(pady=2, fill="x")
        ttk.Button(settings_frame, text="Cancel", command=lambda: settings_window.destroy(), style="TButton").pack(pady=2, fill="x")
